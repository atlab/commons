...
classdef Conditions < dj.Relvar
    ...
    methods
        ...
        function fill(self)
            ...
            tuples(1).condition_pop_str = 'pro(patch.Eye & patch.EyeFrame,''file_num->scan_idx'')';
            tuples(1).condition_params = [];

            tuples(2).condition_num=2;
            tuples(2).condition_name = 'quiet constricting';
            tuples(2).condition_pop_str = 'pro(patch.Eye & patch.EyeFrame,''file_num->scan_idx'')';
            tuples(2).condition_params = [];
            
            tuples(3).condition_num=3;
            tuples(3).condition_name = 'quiet';
            tuples(3).condition_pop_str = 'pro(patch.Recording & patch.Running,''file_num->scan_idx'')';
            tuples(3).condition_params = [];

            tuples(4).condition_num=4;
            tuples(4).condition_name = 'active';
            tuples(4).condition_pop_str = 'pro(patch.Recording & patch.Running,''file_num->scan_idx'')';
