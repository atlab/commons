classdef plots
    ...
    methods(Static)
        function compareTuning
            v0 = pro(bs.VonMises & 'tuning_cond=0' & 'von_p<0.05' & 'von_amp1>0.001','tuning_cond->c0');
            v1 = pro(v0*bs.VonMises & 'tuning_cond=1', 'tuning_cond->c1', 'von_amp1->a1', 'von_p->p1');
            v2 = pro(v0*bs.VonMises & 'tuning_cond=2', 'tuning_cond->c2', 'von_amp1->a2', 'von_p->p2');
            [a1,a2] = fetchn(v1*v2&'animal_id=1469','a1','a2');
            scatter(a1,a2)
        ...
        function BsRaster(varargin)
            for key = fetch(bs.BrainState & varargin)'
                ...
                vm = cell(size(keys));
                bt = cell(size(keys));
                brainState = nan(size(keys));
                for i=1:length(keys)
                    [onset,trialBs] = fetch1(reso.Trial*bs.TrialBrainState & keys(i),'onset','trial_brain_state');
                    ix = times > onset + interval(1) & times < onset + interval(2);
                    bt{i} = bsTrace(ix);
                    brainState(i) = trialBs;
                    vm{i} = cleanVm(ix);
                ...
                vm = [vm{:}];
                vel = [vel{:}];
                [brainState,order] = sort(brainState);
                vm = vm(:,order);
                vel = vel(:,order);
                subplot 131
                ...
                for iTrace = 1:size(traces,2)
                    ...
                    for iTrial = 1:length(keys)
                        [onset,trialBs] = fetch1(reso.Trial*bs.TrialBrainState & keys(iTrial),'onset','trial_brain_state');
                        ix = times > onset + interval(1) & times < onset + interval(2);
