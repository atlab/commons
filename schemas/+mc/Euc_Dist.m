% Plot connection probability as a function of Euclidian distance
nbins = 3;

select = [ss_conn ctl_conn];
figure('OuterPosition',[0 1000 1500 500]);
dist = fetchn(mc.Distances & select,'euc_dist');

% First plot for all connections

Title = 'All Connections';
for i = 1:nbins
    total = fetch (mc.Distances & select & ['euc_dist>' num2str((i-1)*((max(dist)+1)/nbins))] & ['euc_dist<=' num2str(i*((max(dist)+1)/nbins))]);
...
xlabel('Euclidian Distance (um)','FontName','Arial','FontSize',14,'FontWeight','Bold');
