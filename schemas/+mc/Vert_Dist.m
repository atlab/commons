% Plot connection probability as a function of vertical distance
nbins = 3;

select = [ss_conn ctl_conn];
figure('OuterPosition',[0 1000 1500 500]);
dist = fetchn(mc.Distances & select,'vert_dist');

% First plot for all connections
Title = 'All Connections';
for i = 1:nbins
    total = fetch (mc.Distances & select & ['vert_dist>' num2str((i-1)*((max(dist)+1)/nbins))] & ['vert_dist<=' num2str(i*((max(dist)+1)/nbins))]);
...
subplot(1,3,1);
bar(x,y,'BarWidth',1);
text(x,y+0.006,labels,'HorizontalAlignment','center','FontName','Arial','FontSize',12,'FontWeight','Bold');
text(x(end),0.39,['n=' num2str(sum(total_count))],'HorizontalAlignment','center','FontName','Arial','FontSize',12,'FontWeight','Bold');
ylim([0 0.4]);
xlim([0 round(max(dist))]);
xlabel('Vertical Distance (um)','FontName','Arial','FontSize',14,'FontWeight','Bold');
